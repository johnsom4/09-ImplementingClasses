...
class Point(object):
    ...
    def move_to(self, x2, y2):
        ...
        self.y = y2
        self.moves += 1
        self.total += math.sqrt(((self.x - newstart_x) ** 2) + ((self.y - newstart_y) ** 2))
    ...
    def get_distance_from(self, point):
        self.distance = math.sqrt(((self.x - point.x) ** 2) + ((self.y - point.y) ** 2))
    ...
    def halfway_to(self, p2):
        halfway_x = ((p2.x - self.x) / 2) + self.x
        halfway_y = ((p2.y - self.y) / 2) + self.y
        return "Point({}, {})".format(halfway_x, halfway_y)
